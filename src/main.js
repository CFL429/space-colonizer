...
const actions = {
  ...
  screenshot: () => {
    ...
    a.href = url
    a.download = `space-colonizer-${colony.planet.id}-${stamp()}.png`
    a.click()
...
async function boot() {
  ...
  if (!localStorage.getItem('spacecolonizer.seen-help')) {
    hud.toggleHelp(true)
    localStorage.setItem('spacecolonizer.seen-help', '1')
  } else {
...
window.spaceColonizer = { engine, rig, colony, settings, hud, poll, get threads() { return threads } }
